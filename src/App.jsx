...
// --- 1. LÓGICA DE SELECCIÓN Y CONTADOR (RETO) ---
  ...
  const generateChallenge = useCallback(() => {
    ...
    while (interleavedSelection.length < 30 && deckIds.some(id => groupedByDeck[id].length > 0)) {
      ...
      if (groupedByDeck[currentDeckId].length > 0) {
        interleavedSelection.push(groupedByDeck[currentDeckId].shift());
      }
      pointer++;
    }
  ...
  const addPoints = (amount, desc, actionData = null) => {
    ...
    if (newLevel > oldLevel) {
      setLevelDates(prev => ({ ...prev, [newLevel]: new Date().toLocaleDateString() }));
    }
  ...
  const undoAction = (id) => {
    ...
    if (log) { 
      if (!log.actionData && log.amount !== 0) {
          alert("Esta acción es demasiado antigua para deshacer sus cambios específicos de forma segura. Se restaurarán tus puntos.");
      }
...
{/* NAV FOOTER FIXED: Diseño de píldora horizontal para maximizar espacio de pantalla */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-slate-100 p-2 pb-safe z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
        <div className="max-w-md mx-auto flex justify-around items-center">
          
          {/* MAPA */}
          <button 
            onClick={() => setActiveTab('map')} 
            className={`flex items-center gap-2 px-4 py-2 rounded-2xl transition-all duration-300 ${activeTab === 'map' ? 'bg-teal-100 text-teal-700' : 'text-slate-400 hover:text-slate-600'}`}
          >
            <Icon name="MapIcon" size={20} />
            {activeTab === 'map' && <span className="text-xs font-black tracking-wide">MAPA</span>}
          </button>

          {/* TEMAS */}
          <button 
            onClick={() => setActiveTab('syllabus')} 
            className={`flex items-center gap-2 px-4 py-2 rounded-2xl transition-all duration-300 ${activeTab === 'syllabus' ? 'bg-amber-100 text-amber-700' : 'text-slate-400 hover:text-slate-600'}`}
          >
            <Icon name="BookOpen" size={20} />
            {activeTab === 'syllabus' && <span className="text-xs font-black tracking-wide">TEMAS</span>}
          </button>

          {/* PROG */}
          <button 
            onClick={() => setActiveTab('planning')} 
            className={`flex items-center gap-2 px-4 py-2 rounded-2xl transition-all duration-300 ${activeTab === 'planning' ? 'bg-teal-100 text-teal-700' : 'text-slate-400 hover:text-slate-600'}`}
          >
            <Icon name="FileText" size={20} />
            {activeTab === 'planning' && <span className="text-xs font-black tracking-wide">PROG</span>}
          </button>

          {/* PRACT */}
          <button 
            onClick={() => setActiveTab('practico')} 
            className={`flex items-center gap-2 px-4 py-2 rounded-2xl transition-all duration-300 ${activeTab === 'practico' ? 'bg-indigo-100 text-indigo-700' : 'text-slate-400 hover:text-slate-600'}`}
          >
            <Icon name="Target" size={20} />
            {activeTab === 'practico' && <span className="text-xs font-black tracking-wide">PRACT</span>}
          </button>
    ...
    <div className="space-y-6 animate-in slide-in-from-left-4 text-left pb-10">
      ...
      <div className="bg-white/70 backdrop-blur p-6 rounded-[32px] border border-white/50 shadow-sm space-y-4">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { setShowExamFilters(!showExamFilters); setExamCategoryFilter([]); }} className={`flex-1 py-3 rounded-2xl font-black text-[11px] uppercase shadow-md flex items-center justify-center gap-2 active:scale-95 transition-all ${showExamFilters ? 'bg-rose-100 text-rose-700' : 'bg-white text-slate-600 border border-slate-100'}`}>
...
function DeckStudyView({ deck, onBack, addPoints, onUpdateCard, onFinishChallenge }) {
  ...
  return (
    <div className="max-w-xl mx-auto py-10 space-y-8 text-left">
      ...
      {/* LA CLAVE: El atributo key fuerza a React a resetear el componente sin parpadeos */}
      <FlashcardUI 
        key={card.id + idx} 
        card={card} 
        isFlipped={flipped} 
          ...
          <div className="bg-white rounded-[40px] p-8 max-w-3xl w-full shadow-2xl animate-in zoom-in-95 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
              <div className="space-y-4">
                ...
                {/* AQUÍ ESTÁ EL CAMBIO: Usa la piscina autogenerada por seguridad */}
                <div className="grid grid-cols-5 gap-1 p-2 bg-slate-50 rounded-2xl max-h-40 overflow-y-auto border border-slate-100">
                  {topicPool.map(t => (
                    <button key={t.id} type="button" onClick={() => setNewDeckTopics(p => p.includes(t.id) ? p.filter(x => x !== t.id) : [...p, t.id])} className={`p-1.5 rounded-lg text-[8px] font-black border transition-all ${newDeckTopics.includes(t.id) ? 'bg-rose-600 text-white border-rose-600' : 'bg-white text-slate-400 border-transparent'}`}>{t.id}</button>
                  ))}
                </div>
              </div>
              <div className="space-y-4 flex flex-col">
